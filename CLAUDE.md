...
- **State**: `SnapshotProvider` (`src/contexts/SnapshotContext.tsx`), outermost in `Layout`. `useSnapshots()` is metadata only and costs nothing; `useRoster()` triggers the lazy load. Only `/import` never calls it — every other view does when visited, including `CompareView` (its player picker needs the full roster). `CompareContext` itself does not: it validates its at-most-3 stored ids with keyed `db.getPlayer` reads, so mounting it in `Layout` does not cost `/import` a roster read. One roster is in memory at a time.
- **Import** asks which save the file belongs to and what date it is, prefilled from a trailing `DD_MM_YYYY` in the filename. *New save* erases everything — snapshots, league rankings, lists/annotations, compare list, My Team club and squad plan, clearing both the stored values and the providers' live React state so nothing stale lingers in memory. *Same save* adds a snapshot and touches nothing else, other than replacing a snapshot already on that date (the dialog warns, but importing is still the one click). Files missing required columns are refused by name before any write.
- **The compare list is pruned only on initial load**, against keyed `db.getPlayer` reads, never on a snapshot switch — a compared player missing from the newly active snapshot renders as missing rather than being silently dropped from the list.
